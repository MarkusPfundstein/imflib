...
//
// Author: Oboltus, December 2003, Markus Pfundstein, August 2014
//
...
public:
	...
	inline XmlStream(std::ostream&	_s, std::string enc = std::string("UTF-8"))
	: encoding(enc), state(stateNone), s(_s), prologWritten(false) {}
	...
	XmlStream& operator<<(const Controller& controller) {
		...
		case Controller::whatProlog:
			if (!prologWritten && stateNone == state) {
				s << "<?xml version=\"" << versionMajor << '.' << versionMinor << "\" encoding=\"" << encoding << "\"?>\n";
				prologWritten = true;
			}
